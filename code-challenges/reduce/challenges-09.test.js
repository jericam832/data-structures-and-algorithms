...
const countNumberOfElements = (arr) => {
  const result = arr.reduce((counter, idx) => { 
    let total = counter + (idx.a !== 1);
    return total;
  }, 0);
  return result;
};
...
const returnNames = (arr) => {
  let characters = arr.reduce( function (acc,person,idx) {
...
const reversedString = (str) => {
  ...
  const reversedIt = array.reduce((reversed, character) => {
