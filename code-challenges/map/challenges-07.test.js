...
const charCode = (arr) => {
  ...
  for (let i = 0; i < joinedArr.length; i++) {
    let code = joinedArr.charCodeAt(i);
    newArr.push(code);
  }
  return newArr;
};
...
const extractAbilities = (arr) => {
  let newArr = [];
  Object.values(arr).map((power, name) => {
    let thing = power.ability.name;
    newArr.push(thing);
  });
  return newArr;
};
...
const extractStats = (arr) => {
  let newArr = [];
  Object.values(arr).map((stat, i) => {
    let thing = stat.stat.name;
    let total = stat.effort + stat.baseStat;
    let newStat = { "name": thing, "total": total };
    newArr.push(newStat);
  });
  return newArr;
};
